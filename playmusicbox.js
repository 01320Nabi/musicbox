"use strict"
function playMusicBox(sheet, type) {
    ...
    sheet.notes.forEach(note => {
        ...
        if(getFrequency(note.key)) {
            console.log(note.key)
        }
        oscillator.frequency.value = getFrequency(note.key)
        oscillator.type = type
        oscillator.start(speed * (note.msr * sheet.signature * 4 + note.pos))
        oscillator.stop(speed * (note.msr * sheet.signature * 4 + note.pos + note.dur))
        gain.gain.setValueCurveAtTime([note.pwr / 25, 0], speed * (note.msr * sheet.signature * 4 + note.pos), 5)
